...
const statusConfig: Record<string, { label: string; color: string; bg: string }> = {
  ativo: { label: 'Ativo', color: '#8a7560', bg: '#f0ebe4' }, critico: { label: 'Crítico', color: '#c0392b', bg: '#fdecea' },
  esgotado: { label: 'Esgotado', color: '#7f8c8d', bg: '#f0f0f0' }, vencido: { label: 'Vencido', color: '#856404', bg: '#fff3cd' },
  descartado: { label: 'Descartado', color: '#555', bg: '#eee' },
};

const catColors: Record<string, string> = { 'Toxina Botulínica': '#BBA188', 'Preenchimento': '#EBD5B0', 'Bioestimulador': '#1b1b1b', 'Fio de PDO': '#a8906f', 'Skincare/Pele': '#8a7560', 'Descartável': '#95a5a6' };
...
export default function Lotes() {
  ...
  function handleDateChange(field: 'dataFabricacao' | 'dataValidade' | 'dataEntrada', raw: string) { if (!raw) { handleChange(field, ''); return; } const [yearStr, month, day] = raw.split('-'); const safeYear = yearStr ? yearStr.slice(0, 4) : ''; handleChange(field, `${safeYear}-${month ?? ''}-${day ?? ''}`); }

  function openNew() { setSelected(null); setForm(FORM_INITIAL); clearAll(); setIsModalOpen(true); }

  function openEdit(lote: Lote) {
    setSelected(lote);
    setForm({ lote: lote.lote, registroAnvisa: lote.registroAnvisa, produto: lote.produto, categoria: categoryOptions.find(c => c.label === lote.categoria)?.value ?? '', fabricante: lote.fabricante, fornecedor: lote.fornecedor, quantidadeEntrada: String(lote.quantidadeEntrada), dataFabricacao: lote.dataFabricacao, dataValidade: lote.dataValidade, dataEntrada: '', statusLote: statusOptions.find(s => s.label.toLowerCase() === lote.status)?.value ?? lote.status });
    ...
    <Container>
      ...
      <Modal isOpen={isModalOpen} onClose={handleClose} title={selected ? 'Editar Lote' : 'Registrar Novo Lote'} size="lg" footer={<><Button variant="outline" onClick={handleClose}>Cancelar</Button><Button variant="primary" onClick={handleSave}>Salvar Lote</Button></>}>
        <FormGrid>
          ...
          <div style={{ gridColumn: 'span 2' }}><Input label="Nome do Produto *" placeholder="Nome completo do produto..." value={form.produto} onChange={(e) => handleChange('produto', e.target.value)} error={errors.produto} /></div>
          <div style={{ gridColumn: 'span 2' }}><Select label="Categoria *" options={categoryOptions} placeholder="Selecione..." value={form.categoria} onChange={(v) => handleChange('categoria', v)} error={errors.categoria} /></div>
          ...
          <div style={{ gridColumn: 'span 2' }}><Select label="Status *" options={statusOptions} placeholder="Selecione..." value={form.statusLote} onChange={(v) => handleChange('statusLote', v)} error={errors.statusLote} /></div>
        </FormGrid>
      </Modal>
        ...
        {selected && (
          <>
            ...
            <DetailSection>
              ...
              <TimelineList>{selected.usos.map((uso, i) => (<TimelineItem key={i}><TimelineDot /><TimelineContent><TimelineDate>{uso.data}</TimelineDate><TimelineText><strong>{uso.paciente}</strong> — {uso.procedimento}<span style={{ marginLeft: 8, color: '#BBA188', fontSize: '0.8rem' }}>Prof: {uso.profissional}</span><span style={{ marginLeft: 8, color: '#999', fontSize: '0.8rem' }}>{uso.quantidade} {selected.unidade} utilizado(s)</span></TimelineText></TimelineContent></TimelineItem>))}</TimelineList>
              <div style={{ marginTop: 20, overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.86rem' }}>
                  ...
                  <tbody>{selected.usos.map((uso, i) => (<UsageTr key={i}><UsageTd>{uso.data}</UsageTd><UsageTd style={{ fontWeight: 600 }}>{uso.paciente}</UsageTd><UsageTd>{uso.procedimento}</UsageTd><UsageTd>{uso.profissional}</UsageTd><UsageTd>{uso.quantidade} {selected.unidade}</UsageTd></UsageTr>))}</tbody>
                </table>
              </div>
            </DetailSection>
          </>
