...
type LancamentoField =
  | 'tipo' | 'categoria' | 'descricao' | 'valor' | 'data' | 'pagamento';
...
const LANCAMENTO_INITIAL: LancamentoForm = {
  tipo: '', categoria: '', descricao: '', valor: '', data: '', paciente: '', pagamento: '',
};
...
export default function Finance() {
  ...
  const {
    errors:     lancErrors,
    validate:   lancValidate,
    clearError: lancClearError,
    clearAll:   lancClearAll,
  ...
  function handleLancDataChange(raw: string) {
    ...
    handleLancChange('data', `${safeYear}-${month ?? ''}-${day ?? ''}`);
  }

  function handleCloseLancModal() {
    setLancForm(LANCAMENTO_INITIAL);
    lancClearAll();
    setIsModalOpen(false);
  }

  function handleSaveLanc() {
    const isValid = lancValidate({
      tipo:      lancForm.tipo,
      categoria: lancForm.categoria,
      descricao: lancForm.descricao,
      valor:     lancForm.valor,
      data:      lancForm.data,
      pagamento: lancForm.pagamento,
    });
    if (!isValid) return;
  ...
  return (
    <Container>
                ...
                <Tr key={f.id}>
                  ...
                  <Td>
                    <TypeBadge $type={f.type}>
                      <span style={{ fontSize: '0.8rem' }}>{f.type === 'receita' ? '↑' : '↓'}</span>
      ...
      <Modal
        isOpen={isModalOpen}
        onClose={handleCloseLancModal}
        title="Novo Lançamento"
        size="md"
        footer={
          <>
            <Button variant="outline" onClick={handleCloseLancModal}>Cancelar</Button>
            <Button variant="primary" onClick={handleSaveLanc}>Salvar</Button>
          </>
        }
      >
        <FormGrid>
