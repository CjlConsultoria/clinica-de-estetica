...
export default function Reports() {
  ...
  return (
    <Container>
      <Header>
        ...
        <FiltersRow>
          <DropdownWrapper>
            ...
            {openDropdown && (
              <DropdownList>
                {filterPeriods.map(p => <DropdownItem key={p} $active={period === p} onClick={() => { setPeriod(p); setOpenDropdown(false); }}>{p}</DropdownItem>)}
              </DropdownList>
            )}
          </DropdownWrapper>
      ...
      <StatsGrid>
        <StatCard label="Receita Total" value="R$ 462.540" color="#BBA188" icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>} trend={{ value: '+14% vs período ant.', positive: true }} />
        <StatCard label="Total de Sessões" value="562" color="#EBD5B0" icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>} trend={{ value: '+9% vs período ant.', positive: true }} />
        <StatCard label="Ticket Médio" value="R$ 822" color="#8a7560" icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>} trend={{ value: '+4.5% vs período ant.', positive: true }} />
        <StatCard label="Novos Pacientes" value="42" color="#1b1b1b" icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" y1="8" x2="19" y2="14"/><line x1="22" y1="11" x2="16" y2="11"/></svg>} trend={{ value: '+18% vs período ant.', positive: true }} />
      </StatsGrid>
              ...
              <Tbody>
                {topProcedures.map((p, i) => (
                  <Tr key={i}>
                    ...
                    <Td>
                      <Badge $bg={p.growth >= 0 ? 'rgba(187,161,136,0.15)' : 'rgba(231,76,60,0.1)'} $color={p.growth >= 0 ? '#BBA188' : '#e74c3c'}>
