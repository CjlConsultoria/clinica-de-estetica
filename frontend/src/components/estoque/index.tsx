...
import {
  ...
  ToggleGroup, ToggleBtn, FormGrid, CardsGrid, StockCard, StockCardTop, StockCardName,
  StockCardCategory, StockCardBody, StockDetailRow, StockDetailLabel, StockDetailValue,
  StockCardFooter, ProgressBarWrapper, ProgressBar, StockAlertBadge, EmptyState,
  SectionDivider, AlertBanner, AlertBannerIcon, AlertBannerText,
} from './styles';
...
export default function Estoque() {
  ...
  return (
    <Container>
          ...
          {filtered.length === 0 ? (
            ...
            <StockCard key={item.id}>
              ...
              <StockCardBody>
                ...
                <ProgressBarWrapper>
                  <ProgressBar
                    $pct={Math.min((item.quantity / item.maxStock) * 100, 100)}
      ...
      >
        <FormGrid>
          <Input label="Nome do Item" placeholder="Ex: Toxina Botulínica Allergan..." defaultValue={selected?.name} />
          ...
          <Select label="Unidade" options={unitOptions} placeholder="Selecione..." />
          <Input label="Quantidade" type="number" placeholder="0" defaultValue={selected?.quantity?.toString()} />
          <Input label="Estoque Mínimo" type="number" placeholder="0" defaultValue={selected?.minStock?.toString()} />
          <Input label="Estoque Máximo" type="number" placeholder="0" defaultValue={selected?.maxStock?.toString()} />
          <Input label="Preço Unitário (R$)" type="number" placeholder="0,00" defaultValue={selected?.price?.toString()} />
          <Input label="Fornecedor" placeholder="Nome do fornecedor" defaultValue={selected?.supplier} />
          <Input label="Validade" type="date" defaultValue={selected?.expiryDate} />
        </FormGrid>
      </Modal>

      <Modal
        isOpen={isMovModal}
        onClose={() => setIsMovModal(false)}
        title={`Movimentar: ${selected?.name}`}
        size="sm"
        footer={
          <>
            <Button variant="outline" onClick={() => setIsMovModal(false)}>Cancelar</Button>
      ...
      >
        <FormGrid style={{ gridTemplateColumns: '1fr' }}>
          ...
          <Input label="Observação" placeholder="Ex: NF 1234, Procedimento da paciente Ana..." />
        </FormGrid>
      </Modal>
    </Container>
  );
