...
export default function Patients() {
  ...
  return (
    <Container>
      ...
      <StatsGrid>
        <StatCard label="Total de Pacientes" value={mockPatients.length} color="#BBA188" icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>} />
        <StatCard label="Pacientes Ativos" value={ativos} color="#8a7560" icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>} />
        <StatCard label="Inativos" value={mockPatients.length - ativos} color="#EBD5B0" icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="8" r="4"/><path d="M20 21a8 8 0 1 0-16 0"/></svg>} />
        <StatCard label="Novos este mês" value={3} color="#1b1b1b" icon={<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" y1="8" x2="19" y2="14"/><line x1="22" y1="11" x2="16" y2="11"/></svg>} />
      </StatsGrid>
      ...
      <Controls>
        ...
        <FilterRow>
          <DropdownWrapper>
            ...
            {openDropdown === 'status' && (
              <DropdownList>
                {filterStatus.map(s => <DropdownItem key={s} $active={filterSt === s} onClick={() => { setFilterSt(s); setOpenDropdown(null); }}>{s}</DropdownItem>)}
              </DropdownList>
            )}
          </DropdownWrapper>
          ...
          <DropdownWrapper>
            ...
            {openDropdown === 'proc' && (
              <DropdownList>
                {filterProcedure.map(s => <DropdownItem key={s} $active={filterProc === s} onClick={() => { setFilterProc(s); setOpenDropdown(null); }}>{s}</DropdownItem>)}
              </DropdownList>
            )}
          </DropdownWrapper>
                ...
                <Tr key={p.id}>
                  <Td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <Avatar $color={avatarColors[i % avatarColors.length]}>{p.name.split(' ').map(n => n[0]).slice(0, 2).join('')}</Avatar>
                      <PatientInfo>
                  ...
                  <Td>
                    <Badge $bg="rgba(187,161,136,0.15)" $color="#BBA188">{p.procedure}</Badge>
                  </Td>
                  <Td>
                    <Badge $bg={statusColors[p.status].bg} $color={statusColors[p.status].color}>{p.status.charAt(0).toUpperCase() + p.status.slice(1)}</Badge>
                  </Td>
                  <Td>
                    <ActionGroup>
                      <IconBtn title="Editar" onClick={() => { setSelectedPatient(p); setIsModalOpen(true); }}>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={selectedPatient ? 'Editar Paciente' : 'Novo Paciente'} size="lg"
        footer={<><Button variant="outline" onClick={() => setIsModalOpen(false)}>Cancelar</Button><Button variant="primary">Salvar</Button></>}>
        <FormGrid>
          <div style={{ gridColumn: 'span 2' }}><Input label="Nome Completo" placeholder="Nome do paciente..." defaultValue={selectedPatient?.name} /></div>
          <Input label="E-mail" type="email" placeholder="email@exemplo.com" defaultValue={selectedPatient?.email} />
          <Input label="Telefone" placeholder="(00) 00000-0000" defaultValue={selectedPatient?.phone} />
          <Input label="Data de Nascimento" type="date" />
          <Input label="CPF" placeholder="000.000.000-00" />
          <Select label="Status" options={statusOptions} placeholder="Selecione..." />
          <Input label="Indicação" placeholder="Como nos conheceu?" />
          <div style={{ gridColumn: 'span 2' }}><Input label="Observações / Alergias" placeholder="Informações de saúde relevantes..." /></div>
        </FormGrid>
      </Modal>
    </Container>
  );
